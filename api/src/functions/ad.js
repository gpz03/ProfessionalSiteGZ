        ...
        if (backendUrlStr) {
            try {
                ...
                if (backendRes.ok) {
                    ...
                    context.log(`Active Directory proxy returned status ${backendRes.status}, falling back to local host inspection.`);
                }
            } catch (err) {
                context.log(`Active Directory proxy failed (${err.message}), falling back to local host inspection.`);
            }
        }
